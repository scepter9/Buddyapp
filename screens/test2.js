...
function MessageUser({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    newSocket.on('newMessage', (message) => {
      // Only add message to current chat if it belongs to this conversation
      ...
      if (isForThisConversation) {
        ...
        // If the new message is from the other person in this current chat, mark it as read
        if (!isMine) {
          resetUnreadCountForConversation(recipientId, myUserId);
        }
      }
    });
